/**
 * Off-Chain Map Component
 *
 * Generates SVG map client-side for preview/testing.
 * This mimics the on-chain contract SVG generation logic.
 */
...
import GlowSvg from '../../assets/Glow.svg?raw';
...
interface OffChainMapProps {
  progress: AdventureProgress;
}

export function OffChainMap({ progress }: OffChainMapProps) {
...
function renderMapPath(completedLevels: number[], totalLevels: number, tokenId: number, isComplete: boolean) {
  ...
  const glow = extractSvgContent(GlowSvg);
    ...
    <g key="dojo-decoration">
      ...
      {isComplete && (
        <svg
          ...
          width="160"
          height="160"
          viewBox={glow.viewBox}
          dangerouslySetInnerHTML={{ __html: glow.content }}
        />
      )}
